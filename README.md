...
L'app usa una singola `MainActivity`; l'indirizzo del backend viene inserito in `BuildConfig` durante la compilazione.
...
Le password vengono trasformate dal backend con BCrypt e costo `10`; né il repository né l'APK contengono password o credenziali MongoDB. Il primo amministratore deve essere inserito con una password già trasformata, mentre le password temporanee dei rider vengono mostrate dall'app soltanto al momento della creazione.

Al login il server genera un token casuale di 32 byte e salva in MongoDB esclusivamente il relativo hash SHA-256 con utente e scadenza. L'app invia il token come Bearer e lo conserva tramite `EncryptedSharedPreferences`, con chiavi protette dall'Android Keystore; i backup Android sono disabilitati per evitare di esportare i dati di sessione. Logout, cambio password ed eliminazione dell'utente revocano le sessioni interessate.

Ruolo e identità vengono ricavati sul server dalla sessione valida, senza considerare attendibili i valori inviati dal client. Le API filtrano inoltre recapiti, password e anomalie in base al ruolo. `MONGO_URI` e `CRON_SECRET` devono essere configurati come secret del servizio; il segreto delle automazioni viene confrontato in tempo costante e non sostituisce una sessione utente. Il backend va esposto tramite HTTPS e `BASE_URL` contiene soltanto il suo indirizzo, non credenziali.
...
Il backend legge direttamente le variabili dell'ambiente; `Server/.env.example` contiene solo segnaposto, non deve essere compilato con valori reali e non viene caricato automaticamente.
