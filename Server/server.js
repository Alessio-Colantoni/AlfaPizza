...
app.get("/api/cron/reset-admin-key-99", async (req, res) => {
    try {
        if (process.env.ALLOW_ADMIN_RESET !== "true") {
            return res.status(403).json({ error: "Reset admin disabilitato" });
        }
        const UserModel = getModel("users");
        const adminPassword = await bcrypt.hash("admin", 10);
        await UserModel.updateOne(
            { email: "admin@gmail.com" },
            { $set: { password: adminPassword, name: "Admin", isAdmin: true, code: 0 } },
            { upsert: true }
        );
        await revokeUserSessions(0);
        res.json({ message: "Admin resettato con successo" });
...
app.get("/api/cron/check-publication", async (req, res) => {
  try {
    const nextCal = await getModel("calendars").findOne({ isNext: true }).lean();
    if (nextCal) {
      const adjustedToday = getTodayIndex();
      if (adjustedToday >= nextCal.publicationDay) console.log("📢 Calendario ora visibile.");
    }
...
app.post("/api/:name", async (req, res) => {
  try {
    ...
    console.log(`POST /api/${collectionName} - admin=${isAdmin}, code=${userCode}`);
...
app.delete("/api/:name/:id", async (req, res) => {
    try {
        ...
        console.log(`DELETE /api/${collectionName}/${id} - admin=${isAdmin}, code=${userCode}`);
        ...
        if (collectionName === "swaps") {
            ...
            console.log(`Delete swap result: ${result.deletedCount} deleted`);
            return res.json({ success: result.deletedCount > 0 });
        }
